...
<body class="bg-[#F9FAFB] dark:bg-[#0F172A] text-[#0A0A0A] dark:text-[#F1F5F9] transition-colors duration-300">
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        <!-- Board Resolutions and Regulations - Side by Side -->
        <div id="issuancesContainer" class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <!-- Board Regulations Section (Left) -->
            ...
            <!-- Board Resolutions Section (Right) -->
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function applyIssuanceFilters() {
                ...
                // Control layout based on type filter
                const $container = $('#issuancesContainer');
                const $regulationsSection = $('#regulationsSection');
                const $resolutionsSection = $('#resolutionsSection');
                const $regulationsList = $('#regulationsList');
                const $resolutionsList = $('#resolutionsList');

                // Count total items (not just visible) in each section
                const totalRegulations = $regulationsList.find('.issuance-item').length;
                const totalResolutions = $resolutionsList.find('.issuance-item').length;

                if (type === 'regulation') {
                    // Show only regulations at full width
                    $container.removeClass('md:grid-cols-2').addClass('md:grid-cols-1');
                    $regulationsSection.show();
                    $resolutionsSection.hide();
                } else if (type === 'resolution') {
                    // Show only resolutions at full width
                    $container.removeClass('md:grid-cols-2').addClass('md:grid-cols-1');
                    $regulationsSection.hide();
                    $resolutionsSection.show();
                } else {
                    // Show both side by side (All Types) - always show both sections
