...
<div class="p-4 lg:p-6">
    <!-- Page Title -->
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Edit Government Agency</h2>
        <p class="text-gray-600 mt-1">Update agency information</p>
    </div>

    <!-- Form -->
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <form id="editAgencyForm" class="space-y-6">
            <div>
            ...
            <div>
                ...
                <div class="mt-1 flex items-center space-x-5">
                    <div class="flex-1">
                        ...
                        @if($agency->logo)
                            <p class="text-xs text-gray-600 mt-1">Current logo will be replaced if a new one is uploaded.</p>
                        @endif
                    </div>
                    <div id="logoPreview" class="{{ $agency->logo ? '' : 'hidden' }}">
                        @if($agency->logo)
                            <img id="logoPreviewImg" src="{{ asset('storage/' . $agency->logo->file_path) }}" alt="Current logo" class="h-20 w-20 object-contain border border-gray-300 rounded-lg">
                        @else
            ...
            <div class="flex justify-end space-x-4 pt-4 border-t">
                ...
                >
                    <span id="submitBtnText">Update Agency</span>
...
<script>
    ...
    function previewLogo(input) {
        ...
        if (input.files && input.files[0]) {
            ...
            // If no new file, keep showing current logo if it exists
            @if($agency->logo)
                preview.classList.remove('hidden');
            @else
    ...
    document.getElementById('editAgencyForm').addEventListener('submit', async function(e) {
        e.preventDefault();
        ...
        try {
            const response = await axios.post('{{ route("admin.government-agencies.update", $agency->id) }}', formData, {
                headers: {
