...
@section('title', 'Edit Board Member Account')
...
@php
    ...
    // Determine if pre_nominal_title is custom ("Others")
    $standardPreNominals = ['Mr.', 'Ms.', 'Dr.', 'Atty.', 'Engr.', 'Secretary', 'Undersecretary', 'Assistant Secretary', 'Director General', 'Executive Director', 'Attorney'];
    $preNominalValue = $user->pre_nominal_title ?? '';
    $isCustomPreNominal = $preNominalValue && !in_array($preNominalValue, $standardPreNominals);
@endphp
...
<div class="p-4 lg:p-6">
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Edit Board Member Account</h2>
        <p class="text-gray-600 mt-1">Update Board Member account information</p>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 md:p-8">
        ...
        <form id="editBoardMemberForm" class="space-y-4">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        >
                            ...
                            <option value="Board Member" {{ $user->representative_type === 'Board Member' ? 'selected' : '' }}>Board Member</option>
                            <option value="Authorized Representative" {{ $user->representative_type === 'Authorized Representative' ? 'selected' : '' }}>Authorized Representative</option>
                            <option value="Ex-Officio Member" {{ $user->representative_type === 'Ex-Officio Member' ? 'selected' : '' }}>Ex-Officio Member</option>
                        </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <select id="pre_nominal_title" name="pre_nominal_title" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#055498] focus:border-[#055498] outline-none transition">
                                ...
                                <option value="Others" {{ $isCustomPreNominal ? 'selected' : '' }}>Others</option>
                            </select>
                            <div id="pre_nominal_title_custom_wrapper" class="mt-2 {{ $isCustomPreNominal ? '' : 'hidden' }}">
                                <label for="pre_nominal_title_custom" class="block text-xs font-medium text-gray-600 mb-1">Others:</label>
                                <input type="text" id="pre_nominal_title_custom" name="pre_nominal_title_custom" value="{{ $isCustomPreNominal ? $user->pre_nominal_title : '' }}" placeholder="Specify other title" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#055498] focus:border-[#055498] outline-none transition">
                            </div>
                    ...
                    <!-- Birth Date (18 years old allowed, same as register) -->
                    <div>
            ...
            <div class="flex justify-between pt-6 border-t border-gray-200">
                ...
                <div class="ml-auto">
                    ...
                    <button type="submit" id="submitBtn" class="px-6 py-3 text-white rounded-lg font-semibold transition-all duration-300 transform hover:scale-105 shadow-lg hidden" style="background: linear-gradient(135deg, #055498 0%, #123a60 100%);">
                        <span id="submitBtnText">Update Board Member Account</span>
...
<script>
    ...
    // Form submission
    $('#editBoardMemberForm').on('submit', async function(e) {
        e.preventDefault();
        ...
        const formData = {
            ...
            pre_nominal_title_custom: $('#pre_nominal_title').val() === 'Others' ? $('#pre_nominal_title_custom').val() : '',
            first_name: $('#first_name').val(),
            middle_initial: $('#middle_initial').val(),
            last_name: $('#last_name').val(),
            extension_name: ($('#extension_name').val() === 'Others' ? $('#extension_name_custom').val() : $('#extension_name').val()) || '',
