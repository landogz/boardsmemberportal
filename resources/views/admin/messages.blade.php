...
@section('content')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 messages-page-container h-full" style="height: 100%;">
                ...
                <div id="conversationsList" class="md:col-span-1 lg:col-span-1 bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden flex flex-col">
                    <div class="p-3 sm:p-4 border-b border-gray-200">
                        <div class="flex items-center gap-1.5 sm:gap-2 md:gap-2.5">
                            <input type="text" id="conversationsSearch" placeholder="Search names or messages..." class="flex-1 px-2.5 sm:px-3 md:px-4 py-2 sm:py-2.5 text-xs sm:text-sm md:text-sm border border-gray-300 rounded-lg bg-gray-50 text-gray-800 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none min-w-0">
                ...
                <div id="chatArea" class="md:col-span-1 lg:col-span-2 bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden flex flex-col hidden md:flex">
                    ...
                    <div id="activeChat" class="flex flex-col h-full hidden" style="position: relative; display: flex; flex-direction: column;">
                        ...
                        <!-- Message Input -->
                        <div class="p-3 sm:p-4 border-t border-gray-200 bg-gray-50 flex-shrink-0 message-input-container" style="position: relative; margin-top: 0px !important; display: block; visibility: visible;">
                            <!-- Reply Indicator -->
                            ...
                            <form id="messageForm" class="flex items-center space-x-1 sm:space-x-2 flex-nowrap overflow-hidden" style="display: flex; visibility: visible; width: 100%;">
                                ...
                                <input type="text" id="messageInput" placeholder="Type a message..." class="flex-1 min-w-0 px-2 sm:px-4 py-2 sm:py-2 border border-gray-300 rounded-lg bg-white text-gray-800 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none" style="font-size: 16px;">
    ...
    <script>
        ...
        if (!window._adminMessagesCsrf419Interceptor) {
            window._adminMessagesCsrf419Interceptor = true;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // When user returns to tab after switching or idle: refresh list and reload open conversation
            document.addEventListener('visibilitychange', function() {
                if (!document.hidden) {
                    updateUnreadCounts();
                    // Reload the currently open conversation so messages are not stale or stuck on loading
                    if (typeof currentChatUserId !== 'undefined' && currentChatUserId) {
                        if (typeof loadChatConversation === 'function') {
        ...
        function loadChatConversation(userId) {
            ...
            axios.get(`{{ route('messages.conversation', ':userId') }}`.replace(':userId', userId), { timeout: conversationRequestTimeout })
                ...
                .catch(error => {
                    ...
                    messagesArea.innerHTML = '<div class="p-4 text-center text-red-500">' +
                        '<p class="mb-3">' + msg + '</p>' +
                        ...
                        '</div>';
                    messagesArea.querySelector('[data-retry-user-id]').addEventListener('click', function() {
        ...
        function filterConversations(searchTerm) {
            ...
            items.forEach(item => {
                ...
                const lastMsg = (item.querySelector('p.text-gray-500')?.textContent || '').toLowerCase();
