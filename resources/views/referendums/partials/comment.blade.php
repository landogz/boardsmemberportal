@php
    ...
    $isCommenterOnline = $comment->user->is_online ?? false;
@endphp
<div class="fb-comment" id="comment-{{ $comment->id }}" data-comment-id="{{ $comment->id }}">
    <div class="profile-picture-container">
        <img src="{{ $commenterProfileUrl }}" alt="{{ $comment->user->first_name }} {{ $comment->user->last_name }}" class="fb-comment-avatar">
        <div class="{{ $isCommenterOnline ? 'online-indicator' : 'offline-indicator' }}"></div>
    </div>
    <div class="flex-1">
        <div class="fb-comment-content">
            <div class="fb-comment-author">{{ $comment->user->first_name }} {{ $comment->user->last_name }}</div>
            <div class="fb-comment-text-wrapper">
                <div class="fb-comment-text" data-full-text="{{ htmlspecialchars($comment->content, ENT_QUOTES, 'UTF-8') }}">{{ $comment->content }}</div>
        ...
        <div class="fb-comment-actions">
            @if(!$referendum->isExpired())
                <span class="fb-comment-action reply-btn" data-comment-id="{{ $comment->id }}">Reply</span>
            @endif
            @if(Auth::id() === $comment->user_id && !$referendum->isExpired())
                <span class="fb-comment-action edit-comment-btn text-[#1877f2]" data-comment-id="{{ $comment->id }}">Edit</span>
            @endif
            @if((Auth::id() === $comment->user_id || Auth::user()->hasPermission('delete referendum')) && !$referendum->isExpired())
                <span class="fb-comment-action delete-comment-btn text-red-500" data-comment-id="{{ $comment->id }}">Delete</span>
            @endif
            <span class="text-xs text-gray-500 dark:text-gray-400">{{ $comment->created_at->diffInSeconds(now()) < 20 ? 'just now' : $comment->created_at->diffForHumans() }}</span>
        </div>
            ...
            <div class="mt-2">
                ...
                <div class="replies-container hidden mt-2 pl-4 border-l-2 border-gray-200 dark:border-gray-700">
                    @foreach($comment->replies as $reply)
                        @include('referendums.partials.reply', ['reply' => $reply, 'referendum' => $referendum])
                    @endforeach
                </div>
            </div>
        @endif
