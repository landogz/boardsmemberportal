...
<head>
    ...
    <title>Forgot Password - Board Members Portal</title>
    <!-- Favicon -->
...
<body class="bg-[#F9FAFB] dark:bg-[#0F172A] text-[#0A0A0A] dark:text-[#F1F5F9] transition-colors duration-300">
    ...
    <div class="min-h-[50vh] flex items-center justify-center p-4 py-8 sm:py-16">
        <div class="max-w-md w-full bg-white/95 dark:bg-gray-800/95 backdrop-blur-lg rounded-2xl shadow-2xl p-6 sm:p-8 space-y-4 sm:space-y-6">
            <div class="text-center">
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-800 dark:text-gray-100 mb-2">Forgot Password</h1>
                <p class="text-sm sm:text-base text-gray-600 dark:text-gray-400">Enter your email or username to request a reset.</p>
            </div>
            ...
            <form id="forgotPasswordForm" class="space-y-4">
                <div>
                    ...
                    <input
                        ...
                        name="email"
                        required
                        class="w-full px-3 sm:px-4 py-2.5 sm:py-3 text-sm sm:text-base border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#055498] focus:border-[#055498] outline-none transition bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                        placeholder="Enter your email or username"
                    >
                    <span class="text-red-500 text-xs sm:text-sm hidden mt-1 block" id="email-error"></span>
                </div>

                <button
                    type="submit"
                    id="resetBtn"
                    class="w-full text-white py-2.5 sm:py-3 rounded-lg font-semibold text-sm sm:text-base transition-all duration-300 transform hover:scale-105 active:scale-100 shadow-lg touch-manipulation"
                    style="background: linear-gradient(135deg, #055498 0%, #123a60 100%);"
                    onmouseover="if(window.matchMedia('(hover: hover)').matches) this.style.background='linear-gradient(135deg, #123a60 0%, #055498 100%)'"
                    onmouseout="if(window.matchMedia('(hover: hover)').matches) this.style.background='linear-gradient(135deg, #055498 0%, #123a60 100%)'"
                >
            ...
            <div class="text-center space-y-2 pt-2">
                <a href="{{ route('login') }}" class="text-sm font-semibold block sm:inline-block" style="color: #055498;" onmouseover="if(window.matchMedia('(hover: hover)').matches) this.style.color='#123a60'" onmouseout="if(window.matchMedia('(hover: hover)').matches) this.style.color='#055498'">
                    Back to Login
                </a>
                <div class="mt-2 sm:mt-0">
                    <a href="{{ route('landing') }}" class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 inline-block transition-colors">Back to Home</a>
                </div>
            </div>
        </div>
    </div>
